...
function Episodes({ data }: { data: animeDetail }) {
  const title = data.title.userPreferred
    ...
    : data.title.romaji;
  const [ep, Searchep] = useState(24);
  const [searchFor, SetSearch] = useState(0);
    ...
    <>
      {data.episodes.length ? (
        <div className="episodes flex flex-wrap justify-center items-center mt-10 gap-3">
          {data.episodes.length &&
            data.episodes.slice(0, ep).map((ep, i) => (
              <Link
          ...
          {data.episodes.length > ep ? (
            <div className="w-full flex justify-center">
              <button
                ...
                onClick={() =>
                  Searchep((pre) =>
                    pre >= data.episodes.length ? 24 : data.episodes.length
                  )
                }
              >
                {ep <= data.episodes.length ? "Show more" : "Show less"}
              </button>
            </div>
          ) : (
            <></>
